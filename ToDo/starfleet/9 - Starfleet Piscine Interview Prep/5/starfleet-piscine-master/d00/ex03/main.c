...
#include "header.h"
// Optimal utilized space should be somewhere between 70%-75%: 94916 * 1.42857142
...
int main(void)
{
	...
	--------------------*/
	struct s_dict *dict;
	dict = dictInit(CAPACITY);
	
	for (size_t i = 0; arts[i] != NULL; i++){
		dictInsert(dict, arts[i]->name, arts[i]);}
	printf("Price of first test: %d", searchPrice(dict, "Mona Lisa"));
	return (0);
}
