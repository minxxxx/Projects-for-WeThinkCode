...
int main(int argc, char **argv)
{
	...
	if (argc != 2)
		return (printf("\n"));
	stone = genRandomStoneList(atoi(argv[1])); //if param == 0, the length of the list will be between 1 and 10 thousand
...
struct s_stone *genRandomStoneList(int n)
{
	...
	srand(time(NULL));
	if (n <= 0 || n > 1000000){
		n = rand() % 1000000 + 1; //generating int between 1 and 10 thousand
	}
	tmp = NULL;
	begin = NULL;
	repeat = 0;
