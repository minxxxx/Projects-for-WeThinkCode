...
struct s_node *getEventPlace(char *name, struct s_node **places)
{
...
float maxTraffic(struct s_graph *graph, char *eventHere)
{
	struct s_node *event;

	event = getEventPlace(eventHere, graph->places);
