let () =
	...
	let p1 = dalek#exterminate p1 in
	...
	let army = new Army.army (new People.people "GenericPeople") in
	let newArmy =
		let rec loop n army = match n with
		| 0 -> army;
		| _ -> loop (n - 1) (army#add (new People.people "GenericPeople"));
		in
		loop 5 army
	in
	newArmy#foreach (fun elem -> print_endline elem#to_string);
	let deletedArmy =
		let rec loop n army = match n with
		| 0 -> army;
		| _ -> loop (n - 1) (army#remove);
		in
		loop 10 newArmy
	in
	print_endline "deletedArmy : ";
	deletedArmy#foreach (fun elem -> print_endline elem#to_string);

	let army = new Army.army (new Dalek.dalek) in
	let newArmy =
		let rec loop n army = match n with
		| 0 -> army;
		| _ -> loop (n - 1) (army#add (new Dalek.dalek));
		in
		loop 5 army
	in
	newArmy#foreach (fun elem -> print_endline elem#to_string);
	...
	let army = new Army.army (new Doctor.doctor "TheGenericDoctor" 20 (new People.people "Generic Companion") ) in
	let newArmy =
		let rec loop n army = match n with
		| 0 -> army;
		| _ -> loop (n - 1) (army#add (new Doctor.doctor "TheGenericDoctor" 20 (new People.people "Generic Companion") ));
		in
		loop 5 army
	in
	newArmy#foreach (fun elem -> print_endline elem#to_string);
	...
	let gal = (new Galifrey.galifrey)#do_time_war in
	ignore(gal)
