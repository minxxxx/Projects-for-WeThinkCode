...
let rec insert_avl t v =
	...
	| Nil							-> Node (v, Nil, Nil)
	| Node (v', _, _) when v' = v	-> failwith "Already exists"
	| Node (v', a, b) when v' > v	-> balance_avl v' (insert_avl a v) b
	| Node (v', a, b)				-> balance_avl v' a (insert_avl b v)

let rec delete_avl t v =
	match t with
	| Node (v', Nil, Nil) when v' = v	-> Nil
	| Node (v', a, b) when v' = v		-> merge_dst a b v
	| Node (v', a, b) when v' > v		-> balance_avl v' (delete_avl a v) b
	| Node (v', a, b)					-> balance_avl v' a (delete_avl b v)
	| _									-> failwith "Does not exists"
...
let () =
	...
	print_endline "---------";
	test tree;
	let tree = insert_avl tree 5 in
	let tree = insert_avl tree 18 in
	let tree = insert_avl tree 8 in
	let tree = insert_avl tree 9 in
	let tree = insert_avl tree 20 in
	let tree = insert_avl tree (-42) in
	let tree = insert_avl tree 17 in
	print_endline "---------";
	test tree;
	print_endline "---------";
	let tree = delete_avl tree 8 in
	let tree = delete_avl tree 17 in
	let tree = delete_avl tree 1 in
	let tree = insert_avl tree 25 in
