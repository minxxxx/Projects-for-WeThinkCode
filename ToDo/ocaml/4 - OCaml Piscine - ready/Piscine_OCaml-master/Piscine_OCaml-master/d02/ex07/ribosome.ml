...
let main () =
	...
	let rec print_bases_triplets lst =
		let nucleobase_as_string c = match c with
				| A -> "A"
				| T -> "T"
				| C -> "C"
			...
			in
	 		match lst with
				| (a, b, c)::tail -> (Printf.printf "(%s,%s,%s) " (nucleobase_as_string a) (nucleobase_as_string b) (nucleobase_as_string c)) ; print_bases_triplets tail
				| _ -> print_char '\n'
	in *)

	let helix_1 = generate_helix 100000 in
	let rna_1 = generate_rna helix_1 in
	(* print_rna rna_1; *)
	print_endline (string_of_protein (decode_arn rna_1))
